...
export default async function (
  ...
  // fetch owner id
  const owner = newMessage.guild.ownerId;
  // check if the sender is guild owner or not
  try {
    if (newMessage.author.id != owner) {
      // check if message contains link
      if (
        newMessage.content.includes("https://") ||
        newMessage.content.includes("http://") ||
        newMessage.content.includes("www.")
      ) {
        var urls = newMessage.content.match(LinkWhitelist.REGEX);
        if (urls != null) {
          ...
          if (!condition) {
            archived_message = newMessage.content;
            ...
            const botRolePosition =
              newMessage.guild.members.me.roles.highest.position;
            ...
            const embedLog = new EmbedBuilder()
              ...
              .addFields(
                ...
                { name: "Posted at", value: `<#${newMessage.channelId}>` }
              );
            // define embed DM
